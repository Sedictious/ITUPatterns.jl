...
struct APEREC026V01 <: AntennaPattern
    gain::Float64
    η::Float64
    d_to_l::Float64
    g1::Float64
    ...
    """
    function APEREC026V01(gain::Float64, η::Float64=0.7)
        d_to_l = sqrt(10^(gain/10)/(η * π^2))
        φr = 15.85*(d_to_l)^(-0.6) 
        g1 = 32 - 25 * log10(φr)
        φ1 = 0.9 * 114 * (d_to_l)^(-1.09)
        φm = 20/d_to_l * sqrt(gain - g1)
        φmin = d_to_l >= 50 ? max(1, 100 / d_to_l) : max(max(2, 114 * d_to_l^(-1.09), 2.5))
        φb = 10^(42/25)
...
function description(_::APEREC026V01)
    return "Recommendation ITU-R S.465-6 RECEIVING reference Earth station
    antenna pattern for earth stations in FSS in the frequency range
    from 2 to 31 GHz coordinated after 1993."
end

function information(_::APEREC026V01)
    return "For use in coordination and interference assessment.
    Note 5 of the recommendation is applied.
...
function copolar_gain(pattern::APEREC026V01, φ::Float64)
    if φ < 0 || φ > 2π
    ...
    elseif pattern.d_to_l <= 54.5
        ...
        if 0 <= φ < pattern.φm
            return pattern.gain - 2.5 * 1e-3 * (pattern.d_to_l * φ)^2
        elseif pattern.φ1 <= φ < pattern.φr
            return pattern.g1
        else
            return max(32 - 25 * log10(φ), -10)
        end
